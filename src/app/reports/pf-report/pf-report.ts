...
export class PfReport implements OnInit {
  title = 'PF Report';
  ...
  generate() {
    ...
    const now = new Date();
    const wageMonth = this.fromDate || (now.getFullYear() + '-' + String(now.getMonth() + 1).padStart(2, '0') + '-01');
    this.loading = true;
    this.emsService.getPFReport(wageMonth).subscribe({
      next: (res: any) => {
        this.loading = false;
        const data = res?.data || [];
        this.rows = data.map((r: any) => ({
          ...
          month: this.fmtMonth(r.wageMonth),
