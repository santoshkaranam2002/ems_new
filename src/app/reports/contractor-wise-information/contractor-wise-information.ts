...
export class ContractorWiseInformation implements OnInit {
  ...
  generate() {
    ...
    this.emsService.searchContractors().subscribe({
      next: (res: any) => {
        ...
        let list = res?.data || [];
        if (this.selContractor !== 'All Contractors') {
          list = list.filter((c: any) => c.contractorName === this.selContractor);
        }
        this.rows = list.map((c: any) => {
          const valid = c.licenceValidTo || c.labourLicenceValidTo;
          const active = valid ? new Date(valid).getTime() >= Date.now() : true;
          return {
            _cid: String(c.contractorID || ''),
            contractor: c.contractorName || '',
            licence: c.labourLicenceNo || '—',
            validTill: this.fmtDate(valid),
            employees: '…',
            workOrders: '…',
            compliance: '—',
        ...
        this.cdr.detectChanges();
        const cap = Math.min(this.rows.length, 40);
        for (let i = 0; i < cap; i++) {
          const row = this.rows[i];
          const cid = Number(row['_cid']);
          if (!cid) { continue; }
          this.emsService.searchEmployees(cid, undefined, undefined, '', '', 1, 1).subscribe({
            next: (r2: any) => {
              const d = r2?.data || [];
              row['employees'] = String(d.length ? (d[0].totalCount ?? d.length) : 0);
              this.cdr.detectChanges();
            },
            error: () => { row['employees'] = '—'; }
          });
          this.emsService.searchWorkOrders(cid).subscribe({
            next: (r3: any) => {
              row['workOrders'] = String((r3?.data || []).length);
              this.cdr.detectChanges();
            },
            error: () => { row['workOrders'] = '—'; }
          });
        }
      },
