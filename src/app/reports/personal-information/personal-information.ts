    ...
    this.emsService.searchEmployees(this.contractorId(), this.locationId(), undefined, '', '', 1, 500).subscribe({
      next: (res: any) => {
        ...
        this.enrichDetails((row, e, detail) => {
          row['father'] = e.fatherSpouseName || e.FatherSpouseName || '—';
          row['birth'] = this.fmtDate(e.birthDate || e.BirthDate);
          row['sex'] = e.sex || e.Sex || '—';
          row['village'] = e.villageName || e.VillageName || '—';
          row['idProof'] = e.identityProofType || e.IdentityProofType || '—';
          row['category'] = e.empCategoryName || e.EmpCategoryName || e.categoryName || '—';
        });
        this.cdr.detectChanges();
      },
