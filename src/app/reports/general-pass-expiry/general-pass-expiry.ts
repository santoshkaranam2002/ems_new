...
export class GeneralPassExpiry implements OnInit {
  ...
  generate() {
    ...
    let days = 30;
    if (this.toDate) {
      const diff = Math.ceil((new Date(this.toDate).getTime() - Date.now()) / 86400000);
      if (diff > 0) { days = diff; }
    }
    this.loading = true;
    this.emsService.getEmployeePassExpiryReport(days).subscribe({
      next: (res: any) => {
        this.loading = false;
        const data = res?.data || [];
        this.rows = data.map((r: any) => ({
          ...
          passIssue: this.fmtDate(r.issueDate),
          passExpiry: this.fmtDate(r.expiryDate),
          daysLeft: String(r.daysLeft ?? ''),
          status: r.passStatus || ''
        }));
        this.cdr.detectChanges();
      },
