...
export class EmployeeInformation implements OnInit {
    ...
    this.emsService.searchEmployees(this.contractorId(), this.locationId(), undefined, '', '', 1, 500).subscribe({
      next: (res: any) => {
        ...
        this.rows = data.map((r: any) => ({
          ...
          passIssue: '—',
          passExpiry: this.fmtDate(r.passExpiryDate),
          status: r.passStatus || r.empStatus || ''
        }));
        this.enrichDetails((row, e, detail) => {
          if (detail.latestGatePass && detail.latestGatePass.issueDate) {
            row['passIssue'] = this.fmtDate(detail.latestGatePass.issueDate);
          }
        });
        this.cdr.detectChanges();
      },
