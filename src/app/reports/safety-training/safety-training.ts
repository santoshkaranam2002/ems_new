...
export class SafetyTraining implements OnInit {
    ...
    this.emsService.searchEmployees(this.contractorId(), this.locationId(), undefined, '', '', 1, 500).subscribe({
      next: (res: any) => {
        ...
        this.enrichDetails((row, e, detail) => {
          row['trainingId'] = e.trainingID || e.TrainingID || '—';
          row['trainingDate'] = this.fmtDate(e.trainingDate || e.TrainingDate);
          row['status'] = e.safetyTrainingStatus || e.SafetyTrainingStatus || 'NotDone';
        });
        this.cdr.detectChanges();
      },
