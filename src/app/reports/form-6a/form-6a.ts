...
export class Form6A implements OnInit {
  title = 'Form 6A';
  ...
  generate() {
    ...
    const from = this.fromDate || (fy + '-04-01');
    const to = this.toDate || now.toISOString().substring(0, 10);
    this.loading = true;
    this.emsService.getForm6AReport(from, to).subscribe({
      next: (res: any) => {
        this.loading = false;
        const data = res?.data || [];
        this.rows = data.map((r: any) => ({
          ...
          wages: String(r.wages ?? ''),
          ee: String(r.employeeContribution ?? ''),
          er: String(r.employerContribution ?? ''),
          period: this.fmtMonth(r.wageMonth)
        }));
        this.cdr.detectChanges();
      },
