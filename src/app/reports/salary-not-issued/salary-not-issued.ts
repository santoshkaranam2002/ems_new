...
export class SalaryNotIssued implements OnInit {
  ...
  generate() {
    ...
    const nowLbl = new Date().toLocaleDateString('en-IN', { month: 'short', year: 'numeric' });
    this.loading = true;
    this.emsService.getSalaryNotIssuedReport().subscribe({
      next: (res: any) => {
        this.loading = false;
        const data = res?.data || [];
        this.rows = data.map((r: any) => ({
          ...
          month: nowLbl,
          lastPaid: this.fmtMonth(r.lastPaidMonth),
          status: 'Pending'
        }));
        this.cdr.detectChanges();
      },
