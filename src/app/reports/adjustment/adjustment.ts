...
export class Adjustment implements OnInit {
  title = 'Adjustment';
  ...
  generate() {
    ...
    const idStr = prompt('Enter Employee ID to view salary adjustments:');
    if (!idStr) { this.cdr.detectChanges(); return; }
    const empId = Number(idStr);
    if (!empId) { toastError('Invalid Employee ID'); return; }
    this.loading = true;
    this.emsService.getEmployeeById(empId).subscribe({
      next: (res: any) => {
        const e = res?.data?.employee || {};
        const code = e.employeeCode || e.EmployeeCode || ('#' + empId);
        const name = e.employeeName || e.EmployeeName || '';
        const contractor = e.contractorName || e.ContractorName || '—';
        this.emsService.getAdjustmentsByEmployee(empId).subscribe({
          next: (r2: any) => {
            this.loading = false;
            const data = r2?.data || [];
            this.rows = data.map((a: any) => ({
              code: code,
              name: name,
              contractor: contractor,
              month: this.fmtMonth(a.wageMonth),
      ...
      error: () => { this.loading = false; toastError('Employee not found'); this.cdr.detectChanges(); }
    });
  }
