...
export class ContractorWiseReleased implements OnInit {
  ...
  generate() {
    ...
    this.loading = true;
    this.emsService.getReleasedEmployeesReport(this.fromDate || undefined, this.toDate || undefined).subscribe({
      next: (res: any) => {
        this.loading = false;
        const data = res?.data || [];
        this.rows = data.map((r: any) => ({
          ...
          code: r.employeeCode || '',
          name: r.employeeName || '',
          cancelDate: this.fmtDate(r.cancellationDate),
          reason: r.reason || '—',
          status: 'Released'
        }));
        this.cdr.detectChanges();
