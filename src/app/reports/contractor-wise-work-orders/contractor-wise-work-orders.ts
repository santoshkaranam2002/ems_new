...
export class ContractorWiseWorkOrders implements OnInit {
  ...
  generate() {
    ...
    this.loading = true;
    this.emsService.getContractorWiseWorkOrdersReport(this.contractorId()).subscribe({
      next: (res: any) => {
        this.loading = false;
        const data = res?.data || [];
        this.rows = data.map((r: any) => ({
          ...
          wo: r.workOrderNo || '',
          area: r.workArea || '—',
          validTill: this.fmtDate(r.validTill),
          employees: '—',
          status: r.woStatus || ''
        }));
        this.cdr.detectChanges();
