...
export class AssignVillageToEmployee implements OnInit {
  ...
  columns = [{'key': 'code', 'label': 'Emp Code'}, {'key': 'name', 'label': 'Employee'}, {'key': 'contractor', 'label': 'Contractor'}, {'key': 'location', 'label': 'Location'}, {'key': 'status', 'label': 'Status'}];

  popupFields: any[] = [{'key': 'village', 'label': 'Village', 'type': 'select', 'options': []}];
  ...
  ngOnInit(): void {
    ...
    this.emsService.searchVillages('').subscribe({
      next: (r: any) => {
        this.villagesList = r?.data ?? [];
        const f = this.popupFields.find((x: any) => x.key === 'village');
        if (f) f.options = this.villagesList.map((v: any) => v.villageName);
        this.cdr.detectChanges();
      }, error: () => {}
    });
    this.loadData();
  ...
  confirmAction() {
    ...
    const villageId = this.villagesList.find((v: any) => v.villageName === this.form['village'])?.villageID;
    if (!villageId) { toastError('Please select a village.'); return; }
    this.emsService.assignVillage(this.target['_empId'], villageId).subscribe({
      next: () => { this.showModal = false; this.target = null; toastSuccess('Village assigned successfully.'); this.loadData(); },
      error: (err: any) => { toastError(err?.error?.message || 'Village assignment failed.'); }
    });
  }
