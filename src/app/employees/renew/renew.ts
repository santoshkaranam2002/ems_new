...
export class Renew implements OnInit {
  ...
  popupFields: any[] = [{'key': 'validity', 'label': 'Validity (months)', 'type': 'select', 'options': ['12', '6', '24']}, {'key': 'remarks', 'label': 'Remarks', 'type': 'text'}];
  ...
  confirmAction() {
    ...
    const months = Number(this.form['validity'] || 12);
    const remarks = this.form['remarks'] || '';
    this.emsService.getEmployeeById(this.target['_empId']).subscribe({
      next: (res: any) => {
        const gp = res?.data?.latestGatePass;
        const gpId = gp?.gatePassID ?? gp?.GatePassID;
        if (!gpId) { toastError('No gate pass found for this employee.'); return; }
        this.emsService.renewGatePass(gpId, months, remarks).subscribe({
          next: () => { this.showModal = false; this.target = null; toastSuccess('Gate pass renewed successfully.'); this.loadData(); },
          error: (err: any) => { toastError(err?.error?.message || 'Renewal failed.'); }
        });
      },
      error: () => toastError('Could not load employee gate pass details.')
    });
  }
